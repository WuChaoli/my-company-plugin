...
### 根目录定义
**所有路径相对于项目根目录**，即包含 `.git/` 和 `.company/` 的目录。
...
│   ├── static/            # 静态文档（长期维护）
│   └── archive/           # 归档（已完成功能）
└── {子模块目录}/          # 如 Digital_human_command_interface/
    └── src/               # 子模块源码
```

### 重要规则
- ✅ **所有测试文档放在 `docs/` 下**（项目根的 docs/）
- ❌ **禁止在子模块目录创建 `docs/`**
- 📝 测试文档路径统一为 `docs/contexts/YYYY-MM-DD_feature/`
...
| 测试环境 | `docs/static/testing/env.md` | 测试环境配置,包含依赖和数据 | 首次/变更时 |
...
- 测试环境配置统一放在 `docs/static/testing/env.md`,避免重复配置
...
9. 文档结构必须正确：所有测试文档必须放在项目根的 `docs/` 下，禁止在子模块目录创建 `docs/`；测试环境配置统一放在 `docs/static/testing/env.md`
...
- [ ] 检查点记录清晰,包含恢复命令
- [ ] 任务清单更新及时,标注完成时间和状态
- [ ] 文档结构正确：所有测试文档在项目根 `docs/` 下，且测试环境配置在 `docs/static/testing/env.md`
